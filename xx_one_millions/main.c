...
float *points;
int numpoints; // * 3 for xyz

void gen_triangles( int across, int down ) {
	int num_tris = across * down;
	numpoints = num_tris * 3;

	points = (float *)malloc( numpoints * 3 * sizeof( float ) );

	float hspacing = 2.0f / (float)across;
	float vspacing = 2.0f / (float)down;

	int counter = 0;
	for ( int downi = 0; downi < down; downi++ ) {
		for ( int acrossi = 0; acrossi < across; acrossi++ ) {
			// top xyz
			points[counter++] = (float)acrossi * hspacing - 1.0f + hspacing * 0.5f; // x
			points[counter++] =
				(float)downi * vspacing - vspacing * 0.5f - 1.0f + vspacing * 0.5f; // y
			points[counter++] = 0.0f;																							// z
			// left xyz
			points[counter++] =
				(float)acrossi * hspacing - vspacing * 0.5f - 1.0f + hspacing * 0.5f; // x
			points[counter++] =
				(float)downi * vspacing + vspacing * 0.5f - 1.0f + vspacing * 0.5f; // y
			points[counter++] = 0.0f;																							// z
			// right xyz
...
anything here. I defined 'APPLE' in the makefile for OS X */
	...
	window = glfwCreateWindow( 1600, 900, "Hello Triangle", NULL, NULL );
	if ( !window ) {
	...
	gen_triangles( 100, 100 );

	glGenBuffers( 1, &vbo );
	glBindBuffer( GL_ARRAY_BUFFER, vbo );
	glBufferData( GL_ARRAY_BUFFER, numpoints * 3 * sizeof( GLfloat ), points,
								GL_STATIC_DRAW );
