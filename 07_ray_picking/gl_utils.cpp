...
int g_gl_window_width       = 800;
int g_gl_window_height      = 800;
int g_gl_framebuffer_width  = 800;
int g_gl_framebuffer_height = 800;
...
/* We must specify 3.2 core if on Apple OS X -- other O/S can specify
 anything here. I defined 'APPLE' in the makefile for OS X */
  ...
  g_window = glfwCreateWindow( g_gl_window_width, g_gl_window_height, "Extended Init.", NULL, NULL );
  if ( !g_window ) {
    fprintf( stderr, "ERROR: could not open window with GLFW3\n" );
    glfwTerminate();
    return false;
  }
  glfwSetFramebufferSizeCallback( g_window, _framebuffer_size_callback );
  glfwSetWindowSizeCallback( g_window, _window_size_callback );
  glfwMakeContextCurrent( g_window );

  glfwGetWindowSize( g_window, &g_gl_window_width, &g_gl_window_height );
