...
vec3 sphere_pos_wor[] = { vec3( 10.0f, 0.0f, -5.0f ), vec3( 2.0, 0.0, 0.0 ),
...
#define ONE_DEG_IN_RAD ( 2.0 * M_PI ) / 360.0 // 0.017444444
	...
	while ( !glfwWindowShouldClose( g_window ) ) {
		...
		vec3 tp = vec3( 10.0f, 0.0f, -5.0f );
		vec3 cp =
			vec3( sinf( current_seconds ) * 5.0f, 0.0f, cosf( current_seconds ) * 5.0f ) +
			tp;
		print( cp );
		view_mat = look_at( cp, tp, vec3( 0.0f, 1.0f, 0.0f ) );
		glUniformMatrix4fv( view_mat_location, 1, GL_FALSE, view_mat.m );
		//}
